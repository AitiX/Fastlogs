...
| `GET /browse/<appId>/pinned` | все закреплённые логи проекта по всем версиям, новые сверху |
...
### 3d. Закреплённые - `GET /browse/<appId>/pinned`

Все закреплённые (`pinned = 1`) логи проекта **по всем версиям** в одном месте, новые сверху. Это авто-закреплённые (лог, по которому завели задачу в Redmine, закрепляется) плюс закреплённые вручную - иначе они разбросаны по версиям. Ответ - как у списка логов версии: `{ appId, name, logs: [...] }`, где каждый элемент - каталожная запись (вкл. `version`, `pinned`, `status`). Закреплённые логи не протухают (`expires_at = null`), поэтому фильтр живости не нужен. Список ограничен сверху (`PINNED_LIST_LIMIT`, по умолчанию 500). `appId` резолвится через алиасы; неизвестный -> `404`; без viewer-токена -> `401`.
