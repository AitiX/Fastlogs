...
| `GET /browse/<appId>/<version>` | записи: `id, title, time, platform, counts, pinned, sessionId` |
...
Все endpoints каталога (вкл. `?session=` и `/api/search`) под тем же viewer-доступом: `Authorization: Bearer <viewer|admin>` или `?token=<...>`. Без токена -> `401`.
