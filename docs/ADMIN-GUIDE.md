...
Как развернуть сервер FastLogs, зарегистрировать игру, настроить уведомления (sinks), ретеншн, бэкап и безопасность. Пользовательская часть (как разработчику и тестеру) - в [`TEAM-GUIDE.md`](TEAM-GUIDE.md).
...
Сервер: Node.js (>= 18) + SQLite (`better-sqlite3`) за nginx. Зависимостей минимум, дефолты нейтральные (`BASE_URL=http://localhost:8787`), лицензия MIT - можно переиспользовать под свой домен.
...
- Токен показывается **один раз** на stdout. В БД хранится только его sha256-хеш, восстановить нельзя.
- Передавайте токен клиенту через защищённый канал (CI-secret / 1Password), не коммитьте в репозиторий.
- Клиент шлёт его как `Authorization: Bearer <token>`. В Unity - поле `Config > Server > Token`, в GameMaker - макрос `FASTLOGS_TOKEN`.
- Повторный запуск `add-app` для существующего `appId` **обновляет** запись и по умолчанию выпускает новый токен (старый перестаёт работать); сохранить старый - `--keep-token`.
...
- **Pin**: `POST /api/logs/<id>/pin` с `{ "pin": true }` ставит `expiresAt=null` (запись не удаляется); pin **открыт по ссылке** (удобно тестеру). Unpin (`pin:false`) и удаление - **только admin-токен**.
...
- **Секреты** (`ADMIN_TOKEN`, `VIEWER_TOKEN`, `IP_SALT`, ingest-токены, ключи деплоя) - только в `/etc/fastlogs/fastlogs.env` и защищённых хранилищах, **никогда в репозиторий** (см. `.gitignore`).
- **Токены игр** хранятся как sha256-хеш; раздаются клиентам через CI-secret / 1Password. Ротация - повторный `add-app` (новый токен, старый умирает); затем обновить конфиг клиента.
- **IP_SALT** - случайное значение, чтобы хеши IP были необратимы.
- **Доступ к каталогу** `/browse` - под viewer-токеном (team). Удаление и unpin - под admin-токеном.
- **Приватность данных**: логи/скриншоты могут содержать персональные данные -> непубличные короткие ссылки, единый `404` на невалидный id (анти-перебор), ограниченный ретеншн.
- **Ротация ключа деплоя** (был засвечен старый `dossh` root-ключ) - **обязательная процедура**, расписана пошагово в [`../server/deploy/DEPLOY-playjoy.md`](../server/deploy/DEPLOY-playjoy.md): создать unprivileged deploy-пользователя со scoped sudo, новый ed25519-ключ с passphrase, установить публичный ключ, проверить, удалить старый root-ключ, `PermitRootLogin prohibit-password`.
- **PlayJoy-прод** (домен `fastlogs.playjoystudios.com`, дроплет `134.122.49.207`) и миграция со старого `UnityWebServer` - в том же [`DEPLOY-playjoy.md`](../server/deploy/DEPLOY-playjoy.md). Деплой в прод - только после явного sign-off владельца сервиса.
