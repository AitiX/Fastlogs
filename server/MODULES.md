...
- `listVersions(appId): Array<{ version, count, last_at }>` - distinct
  `app_version` values with per-version counts and latest `created_at`.
...
- `logs(id PK, app_id, platform, app_version, device_json, title, ts_utc,
   ...
   expires_at NULL, pinned DEFAULT 0, ip_hash)`.
